function [thetalist, success, thetahis] = IKinBodyIterates2(Blist, M, T, thetalist0, eomg, ev)
...
while err && i < maxiterations
    ...
    for m=1:length(thetalist)
        if m~=length(thetalist)
            fprintf([num2str(thetalist(m)) ', '])
        else
            fprintf([num2str(thetalist(m))])
        end
    end
    ...
    for m=1:length(Vb)
        if m~=length(Vb)
            fprintf([num2str(Vb(m)) ', '])
        else
            fprintf([num2str(Vb(m))])
        end
    end
    fprintf('\n\n')
    fprintf(['angular error magnitude ||omega_b||: ' num2str(norm(Vb(1: 3))) '\n\n'])
    fprintf(['linear error magnitude ||v_b||: ' num2str(norm(Vb(4: 6))) '\n\n'])
end
thetahis=thetahis';
success = ~ err;
end